...
RSpec.describe Spree::Order do
  ...
  let(:drip_shopper_activity) { order.drip_shopper_activity }
  ...
  describe '#update_drip_activity' do
    it "triggers as a cart update_hook if order is not completed" do
      expect(drip_shopper_activity).to(receive(:cart_activity).with('updated'))
      order.recalculate
    end

    it "triggers placed event on complete" do
      expect(drip_shopper_activity).to(receive(:order_activity).with('placed'))
      order.complete!
    end
    ...
    context "completed order" do
      ...
      it "triggers fulfilled event on shipments being shipped" do
        expect(drip_shopper_activity).to(receive(:order_activity).with('updated'))
        expect(drip_shopper_activity).to(receive(:order_activity).with('fulfilled'))
      ...
      it "triggers paid event on payments being captured" do
        expect(drip_shopper_activity).to(receive(:order_activity).with('updated'))
        expect(drip_shopper_activity).to(receive(:order_activity).with('paid'))
